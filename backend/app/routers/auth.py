...
async def login(credentials: LoginRequest):
    ...
    if not verify_password(credentials.password, admin["password_hash"]):
        raise HTTPException(status_code=401, detail="Email ou senha incorretos")
    access_token = create_access_token(data={"sub": admin["id"], "email": admin["email"]}, expires_delta=timedelta(hours=24))
...
async def get_me(admin: dict = Depends(get_current_admin)):
    return AdminUser(id=admin["id"], email=admin["email"], full_name=admin.get("full_name"), created_at=admin["created_at"], last_login=admin.get("last_login"), is_active=admin["is_active"])
